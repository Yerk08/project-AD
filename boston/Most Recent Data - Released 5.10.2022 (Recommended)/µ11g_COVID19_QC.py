...
def daily_QC(data, report_missing=False):
    ...
    assert (data.loc[(data['covid19_complete'].isna() & data['april_18_complete'].isna() & data['nov15_complete'].isna()), 'redcap_repeat_instrument'] == 'covid19_short_survey').all()
    assert (data.loc[(data['covid19_short_survey_complete'].isna() & data['april_18_complete'].isna() & data['nov15_complete'].isna()), 'redcap_repeat_instrument'] == 'covid19').all()
    assert data.loc[(data['covid19_short_survey_complete'].isna() & data['covid19_complete'].isna()), 'redcap_repeat_instrument'].isna().all()
    assert data.loc[(data['covid19_short_survey_complete'].isna() & data['covid19_complete'].isna() & data['nov15_complete'].isna()), 'april_18_complete'].notna().all()
    ...
    assert not data[['covid19_short_survey_complete', 'covid19_complete', 'april_18_complete', 'nov15_complete']].isna().all(axis=1).any()
    assert all(data[['covid19_short_survey_complete', 'covid19_complete', 'april_18_complete', 'nov15_complete']].notna().sum(axis=1) == 1)
...
def R7_QC(r7, report_missing=False):
    """
    Check for data problems and report missing data by variable
    """
    ...
    #Timestamp vars
    assert r7['StartDate'].min() > pd.to_datetime('7/1/2021')
    assert r7['StartDate'].max() < pd.to_datetime('10/1/2021')
    assert r7['RecordedDate'].min() > pd.to_datetime('7/1/2021')
    assert r7['RecordedDate'].max() < pd.to_datetime('10/1/2021')
    date_diff = (r7['RecordedDate'] - r7['StartDate'])  / np.timedelta64(1, 'h')
    assert all(date_diff > 0)
    print('%d StartDate and RecordedDate greater than 24 hours different'
          % sum(date_diff > 24))
    
    #Latitude and longitude within correct limits
    assert all(r7['LocationLatitude'].between(-55, 80) | r7['LocationLatitude'].isna())
    assert all(r7['LocationLatitude'].between(-180, 180) | r7['LocationLatitude'].isna())
    
    #Age in the right range
    assert r7['Age'].between(18, 122).all()
    
    #Vaccination dates
    assert r7['Q22_1'].isin([np.nan, 'January', 'February', 'March', 
                                        'April', 'May', 'June', 'July', 'August', 
                                        'September', 'October', 'November', 
                                        'December']).all()
    assert all(r7['Q22_2'].between(1, 31) | r7['Q22_2'].isna())
    assert all(r7['Q22_3'].between(2021, 2022) | r7['Q22_3'].isna())
    assert r7['DateSingleDose_1'].isin([np.nan, 'January', 'February', 'March', 
                                        'April', 'May', 'June', 'July', 'August', 
                                        'September', 'October', 'November', 
                                        'December']).all()
    assert all(r7['DateSingleDose_2'].between(1, 31) | r7['DateSingleDose_2'].isna())
    assert all(r7['DateSingleDose_3'].between(2021, 2022) | r7['DateSingleDose_3'].isna())
    assert r7['DateFirstDose_1'].isin([np.nan, 'January', 'February', 'March', 
                                'April', 'May', 'June', 'July', 'August', 
                                'September', 'October', 'November', 
                                'December']).all()
    assert all(r7['DateFirstDose_2'].between(1, 31) | r7['DateFirstDose_2'].isna())
    assert all(r7['DateFirstDose_3'].between(2019, 2022) | r7['DateFirstDose_3'].isna())
    assert r7['DateSecondDose_1'].isin([np.nan, 'January', 'February', 'March', 
                                    'April', 'May', 'June', 'July', 'August', 
                                    'September', 'October', 'November', 
                                    'December']).all()
    assert all(r7['DateSecondDose_2'].between(1, 31) | r7['DateSecondDose_2'].isna())
    assert all(r7['DateSecondDose_3'].between(2020, 2022) | r7['DateSecondDose_3'].isna())
    
    #Focus vars
    focus_vars = [col for col in r7.columns if col.startswith('Focus_')]
    assert r7[focus_vars].isin([1,2,3,4,5,6,7, np.nan]).all().all()
    
    #Vaccination recall
    check_vars = [col for col in r7.columns if col.startswith('VaccinePhems_')]
    assert r7[check_vars].isin([1,2,3,4,5,6,7, np.nan]).all().all()
    check_vars = [col for col in r7.columns if col.startswith('Q41_')]
    assert r7[check_vars].isin([1,2,3,4,5,6,7, np.nan]).all().all()
    assert r7.loc[:, 'Q51_1':'Q69'].isin([1,2,3,4,5,6,7, np.nan]).all().all()
    assert r7.loc[:, 'Q67_1':'Q71'].isin([1,2,3,4,5,6,7, np.nan]).all().all()
    check_vars = [col for col in r7.columns
                  if col.endswith('Now') or col.endswith('During')]
    assert r7[check_vars].isin([1,2,3,4,5,6,7, np.nan]).all().all()
    assert r7.loc[:, 'Logisitcs_GetShot':'VaccineSupport_SecuringAppt'].isin([1,2,3,4,5,6,7, np.nan]).all().all()
    
    
    ##### CALCULATED VARIABLES #####
    
    assert all(r7['FIRST_Total'].isna() | r7['FIRST_Total'].between(9, 36))
    assert all(r7['Epworth_Total'].isna() | r7['Epworth_Total'].between(0, 24))
    assert all(r7['CDRISC_10_Total'].isna() | r7['CDRISC_10_Total'].between(0, 40))
    assert all(r7['CDRISC_flexibility'].isna() | r7['CDRISC_flexibility'].between(0, 8))
    assert all(r7['CDRISC_self_efficacy'].isna() | r7['CDRISC_self_efficacy'].between(0, 12))
    assert all(r7['CDRISC_regulate_emotions'].isna() | r7['CDRISC_regulate_emotions'].between(0, 4))
    assert all(r7['CDRISC_optimism'].isna() | r7['CDRISC_optimism'].between(0, 12))
    assert all(r7['CDRISC_cognitive_focus'].isna() | r7['CDRISC_cognitive_focus'].between(0, 4))
    assert all(r7['PSAS_Total'].isna() | r7['PSAS_Total'].between(16, 80))
    assert all(r7['PSAS_Somatic'].isna() | r7['PSAS_Somatic'].between(8, 40))
    assert all(r7['PSAS_Cognitive'].isna() | r7['PSAS_Cognitive'].between(8, 40))
    assert all(r7['MMQ_Ability_total'].isna() | r7['MMQ_Ability_total'].between(0, 80))
    ...
    if report_missing:
        ...
        print('\nR7 MISSING DATA REPORT\n')
        
        skip_vars = []
        for col in r7.columns:
            if col in skip_vars:
                continue
            print('%d missing %s' % (r7[col].isna().sum(), col))
    


def R8_QC(r8, report_missing=False):
    """
    Check for data problems and report missing data by variable
    """
    ...
    #Timestamp vars
    assert r8['round_8_timestamp'].min() > pd.to_datetime('10/27/2021')
    assert r8['round_8_timestamp'].max() < pd.to_datetime('12/10/2021')
    assert r8['date_time_rd8'].min() > pd.to_datetime('10/27/2021')
    assert r8['date_time_rd8'].max() < pd.to_datetime('12/10/2021')
    print('%d round_8_timestamp and date_time_rd8 greater than 24 hours different'
          % sum(abs(r8['round_8_timestamp'] - r8['date_time_rd8']) > pd.Timedelta(24, unit='hours')))
    
    #Check likert et al. responses
    assert r8[[x for x in r8.columns if x.startswith('mem_oct')]].isin([np.nan,1,2,3,4,5]).all().all()
    assert r8[[x for x in r8.columns if x.startswith('pine_')]].isin([np.nan,1,2,3,4,5,6,7]).all().all()
    assert r8['nostalg_oct'].isin([np.nan,1,2,3,4,5]).all()
    assert r8[[x for x in r8.columns if x.startswith('carer_1___')]].isin([np.nan,0,1]).all().all()
    assert r8['carer_2'].isin([np.nan,0,1]).all().all()
    assert r8.loc[:, 'psqi_5a':'psqi_5j'].isin([np.nan,0,1,2,3]).all().all()
    assert r8.loc[:, 'psqi_6':'psqi_9'].isin([np.nan,0,1,2,3]).all().all()
    assert r8[[x for x in r8.columns if x.startswith('isi_')]].isin([np.nan,0,1,2,3,4]).all().all()
    assert r8[[x for x in r8.columns if x.startswith('promis')]].isin([np.nan,1,2,3,4,5]).all().all()
    assert r8[[x for x in r8.columns if x.startswith('gad_')]].isin([np.nan,0,1,2,3]).all().all()
    assert r8[[x for x in r8.columns if x.startswith('pss_')]].isin([np.nan,0,1,2,3,4]).all().all()
    assert r8[[x for x in r8.columns if x.endswith('fear')
               or x.endswith('avoid')]].isin([np.nan,0,1,2,3]).all().all()
    assert r8[[x for x in r8.columns if x.startswith('pid_')]].isin([np.nan,0,1,2,3]).all().all()
    assert r8[[x for x in r8.columns if x.startswith('isdi')]].isin([np.nan,0,1]).all().all()
    assert r8.loc[:, 'highrisk_self':'highrisk_check___8'].isin([np.nan, 0, 1]).all().all()
    assert r8.loc[:, 'highrisk_other_2':'children_feb'].isin([np.nan, 0, 1]).all().all()
    assert r8.loc[:, 'child_ages_oct21___1':'covid_belief'].isin([np.nan, 0, 1]).all().all()
    assert r8['severity_cov'].isin([np.nan,1,2,3,4,5]).all().all()
    assert r8.loc[:, 'cov_long_haul':'perished_2'].isin([np.nan, 0, 1]).all().all()
    assert r8[[x for x in r8.columns if x.startswith('covpos')]].isin([np.nan,1,2,3,4,5]).all().all()
    assert r8['experience'].isin([np.nan,1,2,3,4,5,6,7]).all()
    assert r8['vaccination'].isin([np.nan,0,1]).all()
    
    #PSQI
    assert all(r8['psqi_2'].between(0, 8*60) | r8['psqi_2'].isna())
    assert all(r8['psqi_4'].between(0, 24) | r8['psqi_4'].isna())
    
    #MTQ
    assert r8['mtq_1'].isin([np.nan,0,1]).all()
    assert all(r8['mtq_2'].between(0, 7) | r8['mtq_2'].isna())
    
    #Num children
    assert r8['how_many_kids_oct21'].isin([np.nan,1,2,3,4,5,6,7,8]).all()
    
    #COVID date
    assert all(r8['date_cov'].between(pd.to_datetime('2019/11/01'), pd.to_datetime('2021/12/10')) 
               | r8['date_cov'].isna())
    
    #Vaccination date
    assert all(r8['vacc_date'].between(pd.to_datetime('2020/01/01'), pd.to_datetime('2021/12/10'))
               | r8['vacc_date'].isna())
    
    #Vaccination doses
    assert r8['vacc_doses'].isin([np.nan, 1, 2]).all()
    assert r8['vacc_side_effects'].isin([np.nan, 0, 1, 2, 3]).all()
    assert r8['vacc_plan'].isin([np.nan, 0, 1]).all()
    
    
    ##### CALCULATED VARIABLES #####
    
    assert all(r8['NEG_MEMORY_Total'].isna() | r8['NEG_MEMORY_Total'].between(6, 30))
    assert all(r8['Nostalgia_Total'].isna() | r8['Nostalgia_Total'].between(4, 28))
    assert all(r8['Oct21_ISI_Total'].isna() | r8['Oct21_ISI_Total'].between(0, 28))
    assert all(r8['Oct21_PROMIS_Total'].isna() | r8['Oct21_PROMIS_Total'].between(7, 35))
    assert all(r8['PROMIS_Sleep_Disturbance_Total'].isna() | r8['PROMIS_Sleep_Disturbance_Total'].between(8, 40))
    assert all(r8['PROMISE_SRI_Total'].isna() | r8['PROMISE_SRI_Total'].between(8, 40))
    assert all(r8['Oct21_gad_7_total'].isna() | r8['Oct21_gad_7_total'].between(0, 21))
    assert all(r8['Oct21_PSS_TOTAL'].isna() | r8['Oct21_PSS_TOTAL'].between(0, 40))
    assert all(r8['LSAS_Fear'].isna() | r8['LSAS_Fear'].between(0, 72))
    assert all(r8['LSAS_Anxiety'].isna() | r8['LSAS_Anxiety'].between(0, 72))
    assert all(r8['LSAS_TOTAL'].isna() | r8['LSAS_TOTAL'].between(0, 144))
    assert all(r8['PID_Oct21_Total_Raw_Score'].isna() | r8['PID_Oct21_Total_Raw_Score'].between(0, 75))
    assert all(r8['PID_Oct21_Total_Negative_Affect'].isna() | r8['PID_Oct21_Total_Negative_Affect'].between(0, 15))
    assert all(r8['PID_Oct21_Total_Detachment'].isna() | r8['PID_Oct21_Total_Detachment'].between(0, 15))
    assert all(r8['PID_Oct21_Total_Antagonism'].isna() | r8['PID_Oct21_Total_Antagonism'].between(0, 15))
    assert all(r8['PID_Oct21_Total_Disinhibition'].isna() | r8['PID_Oct21_Total_Disinhibition'].between(0, 15))
    assert all(r8['PID_Oct21_Total_Psychoticisim'].isna() | r8['PID_Oct21_Total_Psychoticisim'].between(0, 15))
    assert all(r8['ISDI_TOTAL'].isna() | r8['ISDI_TOTAL'].between(0, 93))
    assert all(r8['ISDI_Nightmares'].isna() | r8['ISDI_Nightmares'].between(0, 12))
    assert all(r8['ISDI_Initial_Insomnia'].isna() | r8['ISDI_Initial_Insomnia'].between(0, 11))
    assert all(r8['ISDI_Fatigue'].isna() | r8['ISDI_Fatigue'].between(0, 10))
    assert all(r8['ISDI_Nonrestorative_Sleep'].isna() | r8['ISDI_Nonrestorative_Sleep'].between(0, 8))
    assert all(r8['ISDI_Daytime_Disturbances'].isna() | r8['ISDI_Daytime_Disturbances'].between(0, 18))
    assert all(r8['ISDI_Fragmented_sleep'].isna() | r8['ISDI_Fragmented_sleep'].between(0, 9))
    assert all(r8['ISDI_Anxiety_Night'].isna() | r8['ISDI_Anxiety_Night'].between(0, 7))
    assert all(r8['ISDI_Light_Sleep'].isna() | r8['ISDI_Light_Sleep'].between(0, 6))
    assert all(r8['ISDI_Movement_Night'].isna() | r8['ISDI_Movement_Night'].between(0, 6))
    assert all(r8['ISDI_Sensations_Night'].isna() | r8['ISDI_Sensations_Night'].between(0, 6))
    assert all(r8['ISDI_Excessive_Sleep'].isna() | r8['ISDI_Excessive_Sleep'].between(0, 6))
    assert all(r8['ISDI_Irregular_Schedule'].isna() | r8['ISDI_Irregular_Schedule'].between(0, 5))
    assert all(r8['ISDI_Sleep_Paralysis'].isna() | r8['ISDI_Sleep_Paralysis'].between(0, 3))
    assert all(r8['ISDI_Sleep_Hallucinations'].isna() | r8['ISDI_Sleep_Hallucinations'].between(0, 4))
    
    assert all(r8['Oct21_PSQIDURAT'].isna() | r8['Oct21_PSQIDURAT'].between(0, 3))
    assert all(r8['Oct21_PSQIDISTB'].isna() | r8['Oct21_PSQIDISTB'].between(0, 3))
    assert all(r8['Oct21_psqi_2NEW'].isna() | r8['Oct21_psqi_2NEW'].between(0, 3))
    assert all(r8['Oct21_PSQILATEN'].isna() | r8['Oct21_PSQILATEN'].between(0, 3))
    assert all(r8['Oct21_PSQIDAYDYS'].isna() | r8['Oct21_PSQIDAYDYS'].between(0, 3))
    assert all(r8['Oct21_PSQIHSE'].isna() | r8['Oct21_PSQIHSE'].between(0, 3))
    assert all(r8['Oct21_PSQI_sleep_eff'].isna() | r8['Oct21_PSQI_sleep_eff'].between(0, 1))
    ...
    if report_missing:
        ...
        print('\nR8 MISSING DATA REPORT\n')
        
        skip_vars = []
        for col in r8.columns:
            if col in skip_vars:
                continue
            print('%d missing %s' % (r8[col].isna().sum(), col))
    


def R9_QC(r9, report_missing=False):
    """
    Check for data problems and report missing data by variable
    """
    ...
    #Timestamp vars
    assert r9['nov15_timestamp'].min() > pd.to_datetime('2021/11/01')
    assert r9['nov15_timestamp'].max() < pd.to_datetime('2021/12/15')
    assert r9['todays_date'].min() > pd.to_datetime('2021/11/01')
    assert r9['todays_date'].max() < pd.to_datetime('2021/12/15')
    print('%d nov15_timestamp and date_todays_date than 24 hours different'
          % sum(abs(r9['nov15_timestamp'] - r9['todays_date']) > pd.Timedelta(24, unit='hours')))
    
    #Estimation questions
    assert r9['est_us'].isin([np.nan,0,1]).all()
    assert all(r9['est_uscancer'].between(0, np.inf) | r9['est_uscancer'].isna())
    assert all(r9['est_voted'].between(0, np.inf) | r9['est_voted'].isna())
    assert all(r9['est_uscovid'].between(0, np.inf) | r9['est_uscovid'].isna())
    assert all(r9['est_statecovid'].between(0, np.inf) | r9['est_statecovid'].isna())
    assert all(r9['est_nonuscancer'].between(0, np.inf) | r9['est_nonuscancer'].isna())
    assert all(r9['est_nonuscovid'].between(0, np.inf) | r9['est_nonuscovid'].isna())
    
    #COVID variables
    assert r9.loc[:, 'covid_test':'covid_belief'].isin([np.nan, 0, 1]).all().all()
    assert r9['severity_cov'].isin([np.nan,1,2,3,4,5]).all()
    assert r9.loc[:, 'cov_long_haul':'perished_2'].isin([np.nan,0,1]).all().all()
    assert r9['experience'].isin([np.nan,1,2,3,4,5,6,7]).all()
    #COVID dates
    assert all(r9['date_cov'].between(pd.to_datetime('2019/11/01'), pd.to_datetime('2021/12/10')) 
               | r9['date_cov'].isna())
    assert all(r9['vacc_date'].between(pd.to_datetime('2020/01/01'), pd.to_datetime('2021/12/31')) 
               | r9['vacc_date'].isna())
    assert all(r9['vacc_date_boost'].between(pd.to_datetime('2021/01/01'), pd.to_datetime('2021/12/31')) 
               | r9['vacc_date_boost'].isna())
    
    #Vaccination questions
    assert r9[['vaccination', 'vacc_boost', 'vacc_plan_boost', 
               'vacc_plan']].isin([np.nan,0,1]).all().all()
    assert r9['vacc_doses'].isin([np.nan,0,1,2,3]).all()
    assert r9['vacc_side_effects'].isin([np.nan,0,1,2,3]).all()
    assert r9['vacc_side_effects_boost'].isin([np.nan,0,1,2,3]).all()

    
    if report_missing:
    
        print('\nR9 MISSING DATA REPORT\n')
        
        skip_vars = []
        for col in r9.columns:
            if col in skip_vars:
                continue
            print('%d missing %s' % (r9[col].isna().sum(), col))
